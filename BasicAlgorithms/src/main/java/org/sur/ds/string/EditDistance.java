		...
		for(int i = 0;i<strLen1;i++){
			for(int j = 0;j<strLen2;j++){
				if(i==0 && j == 0){
					...
					//Cost to convert String of length 0 to String of length i is i
					costMatrix[i][j] = i;
				}
				else if(j==0){
					...
					//We are now considering adding new character to add to each string
					char c1 = str1.charAt(i);
					char c2 = str2.charAt(j);

					//FIrst figure out the additional cost
					int additionalCost;
					if(c1==c2){
